...
class TweenStagger extends Component {
  constructor(props) {
    super(props);
    this.listItems = [];
    this.staggerTween = new TimelineLite({ paused: true });
    this.state = {
      tweenComplete: false,
  ...
  componentDidMount() {
    this.staggerTween = new TimelineLite({
      paused: true,
  ...
  render() {
    return (
      <React.Fragment>
        <h2>Staggered Tweens</h2>
        <p>This is an example of a staggered tween.</p>
        <div className="btn-group">
          {this.state.tweenComplete === false ? (
            <button
              className="btn btn-info"
              onClick={() => this.staggerTween.play()}
            >
              Open
            </button>
          ) : (
          ...
          {this.state.tweenComplete === true ? (
            <button
              className="btn btn-info"
              onClick={() => this.staggerTween.reverse()}
            >
              Close
            </button>
          ) : (
