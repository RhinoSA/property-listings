...
const mammals = ({ mammals } = this.context);
      ...
      <div className="App">
        <form className="form-inline" onSubmit={this.onSubmit}>
          ...
          <div className="form-group">
            ...
            <SearchLatin
              value={searchTermLatin}
              onChange={this.onSearchChangeLatin}
            />
          </div>
        </form>
        <Table
    ...
    return (
      <React.Fragment>
        <ul className="list-group mt-3">
          {mammals
            .filter(nameSearched(pattern))
            .filter(nameSearchedLatin(patternLatin))
            .map(rows => (
              <li className="list-group-item" key={rows.id}>
                <Link to={`/mammals/${rows.slug}`}>
                  {rows.title} - {rows.latin}
                </Link>
              </li>
            ))}
        </ul>
